...
export default function RegisterPage() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (result?.error) { setError("Account created. Please sign in manually."); setLoading(false); return; }
      ...
      <div className="w-full max-w-sm">
        <div className="a-up mb-8 flex flex-col items-center gap-3">
          ...
          <div className="text-center">
            <h1 className="font-[family-name:var(--font-display)] text-2xl text-stone-900">Create your account</h1>
            <p className="mt-1 text-sm text-stone-500">Free to get started</p>
          </div>
        </div>
        ...
        <div className="a-up d1 rounded-2xl border border-stone-200 bg-white p-7 shadow-xl shadow-stone-200/40">
          ...
          <button type="button" onClick={() => { setGoogleLoading(true); signIn("google", { callbackUrl: "/dashboard" }); }}
          ...
          <div className="relative mb-5 flex items-center gap-3">
            <div className="h-px flex-1 bg-stone-100" /><span className="text-xs text-stone-400">or</span><div className="h-px flex-1 bg-stone-100" />
          </div>

          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label htmlFor="name" className="mb-1.5 block text-sm font-medium text-stone-700">Full name</label>
              <input id="name" name="name" type="text" required className="field" placeholder="Jane Smith" />
            </div>
            <div>
            ...
            <div>
              ...
              <input id="password" name="password" type="password" required minLength={8} className="field" placeholder="Min. 8 characters" />
            </div>
        ...
        <p className="a-up d2 mt-5 text-center text-sm text-stone-500">
          ...
          <Link href="/login" className="font-semibold text-[#E5484D] hover:underline">Sign in</Link>
        </p>
      </div>
    </div>
  );
