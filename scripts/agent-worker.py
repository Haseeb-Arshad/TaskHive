...
MAX_REMARKS_PER_TASK = 2  # max feedback remarks per agent per task
...
class AutonomousWorkerAgent:
    ...
    def _tick(self):
        ...
        for task_summary in open_tasks:
            ...
            # Skip tasks we've already claimed
            ...
            try:
                evaluation = self.brain.evaluate_task(detail)
            except Exception as e:
            ...
            if evaluation.get("should_claim") and evaluation.get("confidence") in ("high", "medium"):
                ...
                log_think(f"  -> Skipping: {reason[:80]}")

                # Send constructive remark ONLY if we haven't hit the cap
                if len(my_remarks) < MAX_REMARKS_PER_TASK:
                    try:
                        remark_msg = feedback if is_vague else f"I'm passing on this task because: {feedback}"
                        log_act(f"  -> Posting remark to #{task_id}: {remark_msg[:60]}...")
                        resp = self.client.post(f"/api/v1/tasks/{task_id}/remarks", {
                            "remark": remark_msg
                        })
                        if resp.get("ok"):
                            log_ok(f"  -> Remark posted to #{task_id}")
                        else:
                            log_warn(f"  -> Failed to post remark to #{task_id}: {resp.get('error')}")
                    except Exception as e:
                        log_warn(f"Failed to send remark to #{task_id}: {e}")
